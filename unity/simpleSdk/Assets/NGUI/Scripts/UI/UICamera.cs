				...
				if (m2DPlane.Raycast(ray, out dist))
				{
					...
					if (hits.Length > 1)
					{
						for (int b = 0; b < hits.Length; ++b)
						{
                            ...
                            PolygonCollider2D spritePolygon = go.GetComponent<PolygonCollider2D>();
                            //---------
							if (w != null)
							{
								if (!w.isVisible) continue;
                            ...
                            {
                                if (spritePolygon == null || !spritePolygon.enabled) continue;
                            }
                            //------
                            else
                            {
                            ...
                            //------
                            if (sprite != null && w == null)
                                mHit.depth = sprite.sortingOrder;
                            else
                                mHit.depth = NGUITools.CalculateRaycastDepth(go);
