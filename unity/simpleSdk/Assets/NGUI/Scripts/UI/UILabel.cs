...
public class UILabel : UIWidget
{
	...
	[HideInInspector][SerializeField] int mMaxLineCount = 0;
	// 0 denotes unlimited
	...
	[HideInInspector][SerializeField] public string id = "NULL";
	#if DYNAMIC_FONT
	...
	public int finalFontSize {
		get {
			if (trueTypeFont)
				return Mathf.RoundToInt (mScale * mFinalFontSize);
		...
		if (mFinalFontSize > 0) {
			...
			for (int ps = mFinalFontSize; ps > 0; --ps) {
				...
				if (mOverflow == Overflow.ShrinkContent && !fits) {
					...
					if (regionX != 1f)
						mWidth = Mathf.RoundToInt (mWidth / regionX);
					mHeight = Mathf.Max (minHeight, Mathf.RoundToInt (mCalculatedSize.y));
					if (regionY != 1f)
						mHeight = Mathf.RoundToInt (mHeight / regionY);
	...
	public string GetUrlAtCharacterIndex (int characterIndex)
	{
		if (characterIndex != -1 && characterIndex < mText.Length - 6) {
			...
			if (mText [characterIndex] == '[' &&
				...
				linkStart = mText.LastIndexOf ("[url=", characterIndex);
				...
				for (int i = 0; i < mTempIndices.size; ++i) {
					if (mTempIndices [i] == currentIndex) {
						...
						if (key == KeyCode.UpArrow)
							localPos.y += def + effectiveSpacingY;
